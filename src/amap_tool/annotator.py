...
class AnnotatorWindow(QMainWindow):
 def __init__(self,region_file,annotation_file=None):
  super().__init__();self.rpath=Path(region_file);self.region=json.loads(self.rpath.read_text());self.out=Path(annotation_file or self.rpath.parent.parent/'annotations'/f"{self.region['region_id']}.json");data=json.loads(self.out.read_text()) if self.out.exists() else {'schema_version':'1.1','region':{'region_id':self.region['region_id'],'source_region_file':str(self.rpath)},'nodes':[],'edges':[],'ignore_regions':[]};self.m=GraphModel(data);self.mode='SELECT';self.sel=None;self.temp=[];self.drag=None;self.dirty=False
 ...
 def set_mode(self,x):
  ...
  if self.mode in ('DRAW_PATH','IGNORE') and self.temp and x!='DRAW_PATH': self.temp=[]
 ...
 def click(self,q,e):
  ...
  if self.sel and self.sel[0]=='edge' and e.modifiers() & Qt.KeyboardModifier.AltModifier:
   ...
   if h and h[0]==self.sel[1]: self.m.insert_point(h[0],h[1],[x,y]); self.changed(); return
  if self.mode=='SPLIT':
   h=self.nearest_edge(x,y)
   if h:self.m.split_edge(*h);self.changed();self.set_mode('SELECT')
   return
  n=self.nearest_node(x,y)
  if n:
   if self.mode=='MERGE' and self.sel and self.sel[0]=='node':self.m.merge_nodes(self.sel[1],n);self.changed();self.set_mode('SELECT')
   else:self.sel=('node',n);self.drag=('node',n);self.drag_before=self.m.snapshot()
  else:
   h=self.nearest_edge(x,y);self.sel=('edge',h[0]) if h else None;self.drag=('point',h[0],h[1]) if h else None;self.drag_before=self.m.snapshot() if h else None
   if h and self.sel and self.sel[0]=='edge' and 0<h[1]<len(self.m.edge(h[0])['polyline'])-1 and math.hypot(self.m.edge(h[0])['polyline'][h[1]][0]-x,self.m.edge(h[0])['polyline'][h[1]][1]-y)<12:
    self.sel=('point',h[0],h[1])
 ...
 def finish(self,q):
  if self.mode=='DRAW_PATH' and len(self.temp)>=2:
   before=self.m.snapshot();a,b=f'n{len(self.m.data["nodes"])+1:06d}',f'n{len(self.m.data["nodes"])+2:06d}';self.m.data['nodes'] += [{'id':a,'x':self.temp[0][0],'y':self.temp[0][1],'type':'endpoint'},{'id':b,'x':self.temp[-1][0],'y':self.temp[-1][1],'type':'endpoint'}];self.m.data['edges'].append({'id':f'e{len(self.m.data["edges"])+1:06d}','start_node':a,'end_node':b,'polyline':self.temp,'path_type':'pedestrian_path','visibility':'visible','confidence':'high','verification_source':'rgb_context'});self.m._commit(before);self.changed();self.set_mode('SELECT')
 ...
 def nearest_edge(self,x,y):
  ...
  for e in self.m.data['edges']:
   for i,p in enumerate(e['polyline']):a.append((math.hypot(p[0]-x,p[1]-y),e['id'],i))
  return (min(a)[1],min(a)[2]) if a and min(a)[0]<22 else None
 def render(self):
 ...
 def delete(self):
  ...
  else:self.m.data['nodes']=[n for n in self.m.data['nodes'] if n['id']!=self.sel[1]]
 ...
 def properties(self):
  ...
  if self.sel[0]=='edge':
   ...
   n=next(n for n in self.m.data['nodes'] if n['id']==self.sel[1]);self.form.addRow('Node ID',QLabel(n['id']));c=QComboBox();c.addItems(['junction','endpoint','boundary']);c.setCurrentText(n.get('type','endpoint'));c.currentTextChanged.connect(lambda v:self.m.set_attr('node',n['id'],'type',v) or self.changed());self.form.addRow('Node Type',c);self.form.addRow('Global X/Y',QLabel(f"{n['x']:.1f}, {n['y']:.1f}"))
 def refresh(self):self.warn.clear();self.warn.addItems(check(self.m.data));s=self.m.stats();self.stat.setText(json.dumps(s,ensure_ascii=False,indent=2));self.properties()
